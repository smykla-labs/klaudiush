...
// markdown footer links, while leaving allow-listed references untouched - the
// CLAUDE.md guidance file, the klaudiush tool, and claude-hooks - including in
...
func containsClaudeAIAttribution(message string) bool {
	...
	// Markdown attribution links. The allow-list is applied per matched link
	// rather than to the whole message: a whole-message check would let a real
	// footer slip through whenever the body also mentions klaudiush or CLAUDE.md
	// elsewhere, so each link is judged on its own.
	for _, link := range claudeLinkPattern.FindAllString(lower, -1) {
		if !isLegitimateClaudeReference(link) {
			return true
		}
	}
